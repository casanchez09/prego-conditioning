...
export default function PregnancyTracker() {
  ...
  const copySession = (day, session) => {
    ...
    const getSets = (exId) => {
      const d = session.data[exId];
      return Array.isArray(d) ? d : (d?.sets || []);
    };
    const getNotes = (exId) => {
      const d = session.data[exId];
      return Array.isArray(d) ? "" : (d?.notes || "");
    };
    ...
    const writeEx = (ex) => {
      ...
      sets.forEach((s, i) => {
        if (s.weight || s.reps) {
          lines.push(`  Set ${i+1}: ${s.weight ? s.weight + "lbs × " : ""}${s.reps || "?"} ${ex.isTimed ? "secs" : "reps"}${s.done ? " ✓" : ""}`);
        }
      });
      const n = getNotes(ex.id);
      if (n) lines.push(`  → ${n}`);
    };
    ...
    if (day === "MON") {
      MON_BLOCKS.forEach(b => {
        ...
        const bn = session.data[`${b.id}_notes`];
        if (bn) lines.push(`Block notes: ${bn}`);
        lines.push("");
      });
    ...
    if (day === "TUE") {
      TUE_SUPERSETS.forEach(ss => {
        lines.push(`── ${ss.label} ──`);
        ss.exercises.forEach(writeEx);
        const bn = session.data[`${ss.id}_notes`];
        if (bn) lines.push(`Block notes: ${bn}`);
        lines.push("");
      });
      lines.push("── Core ──");
      TUE_CORE.forEach(writeEx);
    }

    if (day === "WED") {
      WED_BLOCKS.forEach(b => {
        lines.push(`── ${b.label}: ${b.duration} ──`);
        b.exercises.forEach(writeEx);
        const bn = session.data[`${b.id}_notes`];
        if (bn) lines.push(`Block notes: ${bn}`);
      ...
      const fd = session.data[WED_FINISHER.id] || {};
      if (fd.done) lines.push("Cardio Finisher: completed ✓");
      if (fd.notes) lines.push(`Cardio notes: ${fd.notes}`);
    }

    if (day === "THU") {
      THU_SUPERSETS.forEach(ss => {
        lines.push(`── ${ss.label} ──`);
        ss.exercises.forEach(writeEx);
        const bn = session.data[`${ss.id}_notes`];
        if (bn) lines.push(`Block notes: ${bn}`);
      ...
      THU_CORE.forEach(writeEx);
    }

    if (day === "FRI") {
      lines.push("── Core Circuit ──");
      FRI_CIRCUIT.exercises.forEach(writeEx);
      const fd = session.data[FRI_FINISHER.id] || {};
      if (fd.done) lines.push("Cardio Finisher: completed ✓");
      if (fd.notes) lines.push(`Cardio notes: ${fd.notes}`);
    }
